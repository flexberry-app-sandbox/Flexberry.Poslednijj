...
(
	...
	"ОтчетОЗак_m0" RAW(16) NULL,

	"ОтчетОЗак_m1" RAW(16) NULL,
...
ALTER TABLE "ТЧЗаказ"
	ADD CONSTRAINT "ТЧЗаказ_FЗака_7792" FOREIGN KEY ("ОтчетОЗак_m0") REFERENCES "ЗаказПост" ("primaryKey");

CREATE INDEX "ТЧЗаказ_IОтчет_788" on "ТЧЗаказ" ("ОтчетОЗак_m0");

ALTER TABLE "ТЧЗаказ"
	ADD CONSTRAINT "ТЧЗаказ_FОтче_9067" FOREIGN KEY ("ОтчетОЗак_m1") REFERENCES "ОтчетОЗак" ("primaryKey");

CREATE INDEX "ТЧЗаказ_IОтчет_789" on "ТЧЗаказ" ("ОтчетОЗак_m1");
